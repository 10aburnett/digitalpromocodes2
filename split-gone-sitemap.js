...
const SITEMAP_URL = 'https://whpcodes.com/sitemaps/gone.xml';
const URLS_PER_SITEMAP = 196; // ~2354 URLs / 12 sitemaps
...
async function main() {
  try {
    console.log('Fetching original gone sitemap...');
    ...
    console.log(`Creating ${chunks.length} gone sitemap files...`);

    // Create individual sitemap files
    for (let i = 0; i < chunks.length; i++) {
      const sitemapNumber = i + 1;
      const filename = `gone${sitemapNumber}.xml`;
    ...
    console.log(`\nSuccessfully split gone sitemap into ${chunks.length} files!`);
