...
// Top Whop affiliates from user's list (extracted names)
const topWhops = [
...
async function findMatches() {
  ...
  console.log('=== MATCHES FOUND (Top Whop Affiliates with Promo Codes in Our DB) ===\n');

  const matches: Array<{topWhop: string, ourName: string, ourSlug: string, promoCount: number}> = [];
  ...
  for (const whopName of topWhops) {
    ...
    for (const deal of dealsWithPromos) {
      ...
      // Check for matches (name contains search term or vice versa)
      if (
        (dealNameNorm.includes(searchTerm) && searchTerm.length >= 5) ||
        (dealSlugNorm.includes(searchTerm) && searchTerm.length >= 5) ||
        (searchTerm.includes(dealNameNorm) && dealNameNorm.length >= 5)
      ) {
        matches.push({
          topWhop: whopName,
          ourName: deal.name,
          ourSlug: deal.slug,
          promoCount: deal._count.PromoCode
        });
      }
    }
  }

  // Dedupe by slug and print
  ...
  uniqueMatches.sort((a, b) => a.topWhop.localeCompare(b.topWhop));

  for (const m of uniqueMatches) {
    console.log(`✅ "${m.topWhop}" → ${m.ourName}`);
    console.log(`   slug: ${m.ourSlug} | promos: ${m.promoCount}`);
    console.log('');
  }

  console.log(`\n=== TOTAL UNIQUE MATCHES: ${seen.size} ===`);
  console.log(`\nThese are courses from the top Whop affiliate list that exist in your database WITH promo codes.`);
}
