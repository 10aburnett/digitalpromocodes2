...
/**
 * Build and Publish SEO-Optimized Canonical Graph Script
 *
 * This script builds the SEO-optimized graph from the production database with:
 * - Global in-degree caps to prevent hub dominance
 * - Diversity constraints for categories, brands, and price bands
 * - Anti-popularity penalties to spread link equity
 * - Exploration slots for long-tail discovery
 *
 * Usage:
 * - Manual: npx tsx scripts/build-and-publish-graph-optimized.ts
 * - NPM script: npm run graph:publish-optimized
...
async function main() {
  console.log('🚀 SEO-OPTIMIZED CANONICAL GRAPH BUILD & PUBLISH');
  console.log('===============================================');
  ...
  // Step 2: Build the SEO-optimized graph from production database
  console.log('\\n📊 Building SEO-optimized graph from production database...');
  try {
    execSync('npx ts-node --transpile-only scripts/build-graph-optimized.ts', {
      stdio: 'inherit',
      env: { ...process.env, DATABASE_URL: productionDbUrl }
    });
    console.log('✅ SEO-optimized graph build completed successfully');
  } catch (error) {
    console.error('❌ SEO-optimized graph build failed:', error);
    process.exit(1);
  }
  ...
  // Step 4: Validate SEO-optimized graph file
  ...
  const allRecs = Object.values(graphData).flatMap((d: any) => d.recommendations || []);
  const allAlts = Object.values(graphData).flatMap((d: any) => d.alternatives || []);
  const uniqueRecs = new Set(allRecs).size;
  const uniqueAlts = new Set(allAlts).size;

  // Check for hub dominance
  const recCounts = new Map<string, number>();
  allRecs.forEach(slug => recCounts.set(slug, (recCounts.get(slug) || 0) + 1));
  const topHubs = [...recCounts.entries()]
    .sort(([,a], [,b]) => b - a)
    .slice(0, 5);
  ...
  console.log(`   Unique recommendations: ${uniqueRecs} (diversity: ${(uniqueRecs/allRecs.length*100).toFixed(1)}%)`);
  console.log(`   Unique alternatives: ${uniqueAlts} (diversity: ${(uniqueAlts/allAlts.length*100).toFixed(1)}%)`);
  console.log('   Top recommendation hubs:');
  topHubs.forEach(([slug, count]) => {
    console.log(`     ${slug}: ${count} times (${(count/allRecs.length*100).toFixed(1)}% of all recs)`);
  });
  ...
  // Check for excessive hub dominance (SEO warning)
  const maxHubPercentage = topHubs.length > 0 ? (topHubs[0][1] / allRecs.length * 100) : 0;
  if (maxHubPercentage > 5) {
    console.log(`   ⚠️  WARNING: Top hub appears in ${maxHubPercentage.toFixed(1)}% of recommendations (may impact SEO diversity)`);
  }
  ...
  const versionedFilename = `neighbors-optimized-${timestamp}-${commitSha}.json`;
  const latestFilename = 'neighbors-latest.json';

  console.log(`\\n📤 Publishing SEO-optimized graph via ${publishConfig.method}...`);
  ...
  console.log('\\n✅ SEO-OPTIMIZED GRAPH PUBLISH COMPLETED SUCCESSFULLY!');
  console.log('\\n🔧 Environment Variables to Set:');

  if (publishConfig.method === 'file') {
    console.log('   NEXT_PUBLIC_GRAPH_URL=/data/graph/neighbors.json  # (default, can omit)');
    console.log(`   NEXT_PUBLIC_GRAPH_VERSION=${timestamp}-${commitSha}  # (cache buster)`);
  } else {
  ...
  console.log('   3. Verify recommendations are now diverse and SEO-optimized');
  console.log('   4. Check that no single whop dominates too many recommendation slots');
  console.log('\\n🎉 All environments will now use the same SEO-optimized canonical graph!');
}
...
async function publishToFile(versionedFilename: string, latestFilename: string) {
  ...
  console.log(`   ✅ SEO-optimized graph available at: /data/graph/neighbors.json`);
...
async function publishToS3(versionedFilename: string, latestFilename: string, config: PublishConfig) {
  ...
  try {
    ...
    console.log(`   ✅ SEO-optimized graph available at: ${config.baseUrl}/graph/${latestFilename}`);
...
async function publishToGitHub(versionedFilename: string, latestFilename: string) {
  ...
  try {
    ...
    execSync(`git commit -m \"feat: update SEO-optimized canonical graph ${versionedFilename}

🤖 Auto-generated SEO-optimized graph from production database
- Built at: ${new Date().toISOString()}
- Total whops: ${Object.keys(JSON.parse(readFileSync(GRAPH_FILE_PATH, 'utf8'))).length}
- Optimized for diversity and reduced hub dominance
- Commit: ${process.env.GITHUB_SHA || 'local'}\"`, { stdio: 'inherit' });

    console.log('   ✅ SEO-optimized graph committed to repository');
...
export { main as buildAndPublishOptimizedGraph };